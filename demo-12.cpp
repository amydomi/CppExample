...
int main()
{
	...
	// Unicode字符集类型，C++11才支持的数据类型,g++编译需要加 -std=c++0x才能编译通过
	//cout << "wchar_t\t\t" <<sizeof(char16_t) << "byte\tbit" <<  sizeof(char16_t) * 8 << endl;
	//cout << "wchar_t\t\t" <<sizeof(char32_t) << "byte\tbit" <<  sizeof(char32_t) * 8 << endl;
